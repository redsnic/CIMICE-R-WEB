...
reactive({
    ...
    rv$g <- out$topology
    rv$weights <- out$weights
    rv$labels <- out$labels
    # prepare output
    rv$gg <- renderPlot({draw_ggraph(rv$g, rv$weights, rv$labels)})
    rv$nd3 <- renderForceNetwork({draw_networkD3(rv$g, rv$weights, rv$labels)})
    rv$vis <- renderVisNetwork({draw_visNetwork(rv$g, rv$weights, rv$labels, original_dataset=dataset)})
    rv$dot <- renderPrint({cat(to_dot(rv$g, rv$weights, rv$labels))})
}) 
```
